//! A platform agnostic async driver to interface with the QMC5883L magnetometer.
//!
    ...
    I2C: I2c<Error = E>,
{
    ...
    pub async fn set_field_range(&mut self, rng: FieldRange) -> Result<(), E> {
    ...
    pub async fn set_oversample(&mut self, osr: OversampleRate) -> Result<(), E> {
        let ctrl1 = self.read_u8(Register::CONTROL1).await?;
    ...
    pub async fn set_output_data_rate(&mut self, odr: OutputDataRate) -> Result<(), E> {
        let ctrl1 = self.read_u8(Register::CONTROL1).await?;
    ...
    pub async fn continuous(&mut self) -> Result<(), E> {
        let ctrl1 = self.read_u8(Register::CONTROL1).await?;
    ...
    pub async fn enable_interrupt(&mut self) -> Result<(), E> {
        self.write_u8(Register::CONTROL2, CTRL2_ROL_PNT).await
    }

    /// Disable interrupt pin.
    pub async fn disable_interrupt(&mut self) -> Result<(), E> {
        self.write_u8(Register::CONTROL2, CTRL2_ROL_PNT | CTRL2_INT_ENB)
            .await
    }

    /// Read temperature sensor; temperature coefficient is about 100 LSB/°C.
    pub async fn temp(&mut self) -> Result<i16, E> {
    ...
    pub async fn mag(&mut self) -> Result<(i16, i16, i16), Error<E>> {
    ...
    async fn read_u8(&mut self, reg: Register) -> Result<u8, E> {
        let buf: &mut [u8; 1] = &mut [0];
        self.i2c.write_read(I2C_ADDRESS, &[reg as u8], buf).await?;
        Ok(buf[0])
    }

    async fn write_u8(&mut self, reg: Register, v: u8) -> Result<(), E> {
        self.i2c.write(I2C_ADDRESS, &[reg as u8, v]).await
    }
}
